...
describe('bluefile.BlueFileReader', () => {
  ...
  it('should parse type 1000 double data', async () => {
    ...
    expect((hdr!.dview as Float64Array)[0]).to.equal(1);
    expect((hdr!.dview as Float64Array)[1]).to.equal(0.9980267284282716);
    expect((hdr!.dview as Float64Array)[2]).to.equal(0.9921147013144778);
    expect((hdr!.dview as Float64Array)[4093]).to.equal(0.9048270524660175);
    expect((hdr!.dview as Float64Array)[4094]).to.equal(0.9297764858882493);
    expect((hdr!.dview as Float64Array)[4095]).to.equal(0.9510565162951516);
  });
  ...
  it('should read bluefile ascii keywords from HTTP', async () => {
    ...
    while (i <= 100) {
      ...
      expect((hdr!.ext_header as Record<string, unknown>)[key]).to.equal(value);
      i++;
    }
  });
  ...
  it('should read all keywords as Array (list) from HTTP', async () => {
    ...
    expect(hdr).to.not.equal(null);
    const keywords = [
      { tag: 'B_TEST', value: 123 },
      { tag: 'I_TEST', value: 1337 },
      { tag: 'L_TEST', value: 113355 },
      { tag: 'X_TEST', value: 987654321 },
      { tag: 'F_TEST', value: 0.12345000356435776 },
      { tag: 'D_TEST', value: 9.87654321 },
      { tag: 'O_TEST', value: 255 },
      { tag: 'STRING_TEST', value: 'Hello World' },
      { tag: 'B_TEST2', value: 99 },
      { tag: 'STRING_TEST2', value: 'Goodbye World' },
    ];
    const extHeader = hdr!.ext_header as Array<{ tag: string; value: unknown }>;
    for (let i = 0; i < keywords.length; i++) {
      expect(extHeader[i]!.tag).to.equal(keywords[i]!.tag);
      expect(extHeader[i]!.value).to.equal(keywords[i]!.value);
    }
  });
  it('should parse scalar packed data', async () => {
    ...
    expect((hdr!.dview as BitArray).getBit(0)).to.eql(0);
    expect((hdr!.dview as BitArray).getBit(1)).to.eql(1);
    expect((hdr!.dview as BitArray).getBit(2)).to.eql(0);
    expect((hdr!.dview as BitArray).getBit(3)).to.eql(0);
    expect((hdr!.dview as BitArray).getBit(4)).to.eql(0);
    expect((hdr!.dview as BitArray).getBit(5)).to.eql(0);
    expect((hdr!.dview as BitArray).getBit(6)).to.eql(1);
    expect((hdr!.dview as BitArray).getBit(7)).to.eql(0);
  });
});
